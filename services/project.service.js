...
const createAndSaveProject = async (projectName) => {
    try {
        ...
        const errorMessage = error.code === 11000 ? `Project by name ${projectName} already exists`: error.message;
...
const findAllProjects = async () => {
    try {
        ...
        throw new CustomError(500, `Internal server error: Unable to find projects`);
    }
}

const findProjectById = async(projectId, next) => {
    try {
        const isValidId = mongoose.isValidObjectId(projectId);
        if (isValidId) {
            const result = await ProjectModel.findById(projectId).populate('floorPlans');
            if (result) {
                return result;
            }
...
const patchProjectAndSave = async (projectId, newProjectName) => {
    try {
       const result = await ProjectModel.findByIdAndUpdate(projectId, { name: newProjectName }, {new: true});
       if (result) {
           return result;
       }
       throw new CustomError(404, `Project with id: ${projectId} does not exist`);
